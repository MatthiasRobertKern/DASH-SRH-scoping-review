...
#Data
data = pd.read_stata("RevisedData_long.dta", index_col=None)



df = data[['Number', 'Title', 'Year', 'Author', 'Journal', 'Country', 'Age', 'Gender', 'School', 'AdditionalFactors', 'Duration', 'Frequency', 'DeliveryAgent', 'Setting', 'Type', 'Outcome', 'Finding', 'FollowUp', 'Region']]
for item in df.keys():
    print(item)
    print(df[item])
...
additionalfactors = st.multiselect('Additonal factors', ['All'] + df['AdditionalFactors'].unique().tolist(), default='All')
...
if 'All' not in additionalfactors:
    df_filtered = df_filtered[(df['AdditionalFactors'].isin(additionalfactors))]
...
df_filtered = df_filtered[['Number', 'Author', 'Year', 'Title']]
